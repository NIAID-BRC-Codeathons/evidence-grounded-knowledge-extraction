...
Give it an organism, some genes, and a data type, and get back a deduplicated,
...
- [Data types](#data-types)
  ...
  - [Glycosylation sites (`glycosylation`)](#glycosylation-sites-glycosylation)
  - [Host-virus interactions (`host-virus`)](#host-virus-interactions-host-virus)
  - [Adding your own](#adding-your-own)
- [Choosing collections](#choosing-collections)
- [Choosing a generator](#choosing-a-generator)
- [What it does to the data](#what-it-does-to-the-data)
  - [What the columns mean](#what-the-columns-mean)
  - [The Assertion column](#the-assertion-column)
  - [Mutations written in prose](#mutations-written-in-prose)
  - [Row flags](#row-flags)
  - [Passage-level provenance](#passage-level-provenance)
  - [How retrieval works](#how-retrieval-works)
  - [Retrieval depth](#retrieval-depth)
  - [Provenance](#provenance)
- [Notes on this deployment](#notes-on-this-deployment)
...
Exact output varies between runs, since the underlying models are
nondeterministic.

## How it works

One pipeline runs behind both front ends: it retrieves passages, asks a model
to fill a schema, then cleans up what comes back before anything is shown.

Tech stack:

- Language: Python 3.9 or later, with 3.12 pinned in `.python-version`
- Packaging: a uv workspace at the repository root with one member, `litrag/`, and a committed `uv.lock`
- Build backend: setuptools, exposing the `litrag` console script
- CLI: Typer
- Web UI: FastAPI served by uvicorn, with pydantic request models through FastAPI, serving plain HTML, CSS, and JavaScript from `litrag/litrag/web/`, with no front-end framework
- Retrieval client: httpx against the BV-BRC RAGStack API
- Generation: any OpenAI-compatible chat endpoint
- Batch input: TSV, CSV, JSON, or YAML, with PyYAML reading the YAML case
- Testing: pytest over recorded fixtures

| Module | What it owns |
|---|---|
| `cli.py` | Typer commands |
| `server.py` | FastAPI routes and static UI |
| `pipeline.py` | `run_query`, the one entry point both front ends use |
| `client.py` | `RagStackClient` over the RAGStack HTTP API |
| `llm.py` | OpenAI-compatible generator client and presets |
| `extract.py` | Parses the model answer into rows, citations, flags |
| `normalize.py` | Canonical forms for genes, mutations, drugs, sites |
| `dedup.py` | Merges duplicate facts, keeps variants |
| `provenance.py` | Annotation and provenance columns |
| `formats.py` | Table, TSV, CSV, JSON, JSONL, Markdown renderers |
| `templates.py` | Server template registry |
| `local_templates.py` | User and built-in local data types |
| `glossary.py` | Column and flag definitions |
| `collections.py` | Corpus registry |
| `config.py` | API key and base URL resolution |
| `batch.py` | Multi-query runs with resume |
| `prompts.py` | Local prompt construction |

```mermaid
flowchart LR
    CLI[CLI] --> Pipeline
    WebUI[Web UI] --> Pipeline
    Pipeline --> RagClient[RAGStack client]
    Pipeline --> Generator
    Generator --> Extract
    Extract --> Normalize
    Normalize --> Dedup
...
.
├── README.md              # this file
├── ARCHITECTURE.md        # how the package is wired, for maintainers
├── WALKTHROUGH.md         # what a curated table is and how to audit a claim
├── LICENSE                # Apache License 2.0
├── pyproject.toml         # uv workspace root, no package of its own
├── uv.lock                # committed workspace lockfile
└── litrag/                # the litrag package and its own project files
    ├── litrag/            # the litrag package
    │   └── web/           # static HTML, CSS, and JavaScript for the web UI
    ├── tests/             # pytest suite over recorded fixtures
    ├── examples/          # sample batch input
    └── README.md          # the package's own short README
```
...
With [uv](https://docs.astral.sh/uv/) (recommended), no manual virtual
environment and no activation step:
...
`uv run` works from the repository root or from `litrag/`. It re-syncs the
...
Requires Python 3.9 or later. The repository pins 3.12 for development in
`.python-version`.
...
Never commit a key. `litrag/.gitignore` already covers `config.toml` and
`.env`.
...
litrag batch litrag/examples/queries.tsv -o curated.tsv -j 4
...
`data_type`, `other_terms`, `top_k`, `collection`. Common aliases (`species`,
`gene`, `type`) are accepted. Only `organism` is required.
...
Every specification is validated before any request is sent, so a typo in row
40 does not surface after 39 successful queries. Results merge into one table
and deduplicate across queries: a fact found by three searches becomes one row
with `_n_support = 3`.

Each completed query is appended to `<output>.progress.jsonl` as it finishes,
including its extracted rows. `--resume` skips what is already done and
...
Four come from the server (`/v1/prompt-templates`), versioned and hashed
there. Three are defined by LitRAG.
...
defined in `litrag/litrag/local_templates.py` instead, which works because
local generation already builds its prompt from a template declaration.
A `--llm server` selection for this type fails with an explanation rather
than a server 404, and the UI disables the search button for that pairing.

Rules specific to this type, added to the prompt from the declaration:

- One row per strain-antibiotic pair.
- MIC recorded verbatim with its unit and inequality: `>32 mg/L`,
  `≤0.125 mg/L`, `2 µg/mL`. No unit conversion.
...
The merge identity is organism, strain, and antibiotic. Accession and BioSample
...
same drug (`Ampicillin sodium` equals `Ampicillin`, `Ciprofloxacin (CIP)`
equals `Ciprofloxacin`), and `R` equals `Resistant` so wording differences are
not reported as conflicts. A genuine disagreement is: two papers calling the
same strain-drug pair S and R produce one row flagged `merged_variants:SIR`
with both values kept, which is what CLSI-versus-EUCAST breakpoint differences
look like in practice.

A row with neither an MIC nor an SIR names a strain and a drug without
reporting a result, and is dropped as evidence-free.
...
Aliases: `glycosylation`, `glyco`, `glycan`, `glycosite`. Local generator
only, for the same reason as `ast`.
...
The protein is always named, since a site without one cannot be interpreted,
and the strain is recorded wherever a source gives it, as written
...
Identity is organism, protein, strain, and site, since numbering is
strain-dependent, so position 146 on H3N2 HA is not the same site as position
146 on H1N1 HA. Glycan, method, and effect are things observed about a site,
not part of what identifies it, so two papers characterising `N234`
differently merge into one row with both glycans listed. `N-linked` and
`N-glycosylation` are the same linkage. `N-linked` versus `O-linked` on one
site is a real conflict and is flagged.

The linkage is recorded only when the source states it, never inferred from
the residue, and an empty Glycan means the glycan was not characterised, not
that the site is unglycosylated.
...
The curated chain is viral protein to interaction type to host protein to
...
verb that fits none of the six is kept as the source wrote it rather than
filed under the nearest.

The relation is directional, unlike a protein-protein pair: the viral protein
acts on the host target and the pair is never reordered. The verb is part of
the claim, so binding STAT1 and degrading it are two findings. The consequence
is not: two papers reporting different downstream effects of one interaction
merge, with both kept.
...
| `Dengue`, `Influenza_2024_2025`, `Glyco` | Team-curated corpora | 382 to 3,064 chunks |
| `all` | Every corpus in one request | 54.3M chunks |

The list is read from the server, so corpora added by the team appear without
a code change. Empty indexes are filtered out: the registry also lists a raw
...
are active, `all` fails and names them rather than silently searching a
subset, since a quiet gap in coverage is worse for curation than an error.
...
recorded in `_collection` as `open-access+asm-semantic`. A single collection
is still sent as the scalar `collection` field, so those responses stay
identical to a plain single-corpus request.

Unknown ids are rejected before any request is made, and the corpus set is
part of a query's identity, so `--resume` re-runs rather than reusing an
answer from a different corpus. ASM titles carry inline HTML, which is
stripped before display and export.
...
field, but passing one returns HTTP 200 with an empty answer and zero
sources, no error. LitRAG never sends it. That is why the local path splits
retrieval from generation.

What that costs. On the local path LitRAG owns the prompt, so the server's
versioned template hash no longer describes what was sent. The prompt is
still built from the server's template declaration, its label and columns, so
a template added server-side works everywhere without code changes.
Provenance records `_generator`, `_llm_endpoint`, and a `_prompt_hash` of the
exact prompt, alongside the declaration's id, version, and hash. One upside:
because the prompt is ours, View request can actually show it, which the
hosted path cannot.
...
Slots are validated locally, turning a server 422 into a message before the
call.
...
finding. Rows with nothing outside their subject columns are removed and
counted in the summary. `--keep-empty` retains them, flagged.

Citations are resolved to real papers, and to the passage behind them. `[1]`
maps to the first retrieved source, expanded to PMID, PMCID, DOI, journal,
year, and first author. Chunks of one paper collapse to one citation, but
every supporting passage is kept: three chunks of one paper are three pieces
of evidence, and the chunk is what the model actually read. When the model
cites by name instead of by number, matching falls back to the first author
only, since `X et al.` means X is first, and a looser rule mis-attributes
claims. A reference that matches nothing is flagged `citation_not_in_sources`
rather than guessed at. In practice this catches the model citing a paper it
read in another paper's bibliography.
...
forms, and parenthetical glosses like `Ser315Thr (S315T)`, so those become one
row with `_n_support = 3` and the union of their citations. Protein
interactions are matched unordered, since A-B is B-A. Promoter positions keep
their sign: `c-15t` and `c15t` stay distinct.

Disagreement stays visible. When merged rows differ on a non-identity column,
every value is listed in the cell, separated by `; `, and the row is flagged
`merged_variants:<column>`. A merge must not present one paper's qualifier as
every source's finding. Flat formats (TSV, CSV, Markdown) join the same way.
JSON keeps the representative value and a structured `variants` list, since it
can represent both.

### What the columns mean

Every column header in the web UI carries its definition: hover it, or tab to
it and the tooltip opens. Flag chips explain themselves the same way. The
definitions live in `litrag/litrag/glossary.py`, so the CLI serves the same
text:
...
Server templates publish column names but no value vocabulary, so
`Assertion` was undefined and each generator invented its own meaning: Qwen
echoed the instruction to report only what sources state and wrote `Stated`
in every row, Llama wrote evidence types, and the hosted path writes
confidence grades like `High confidence`.
...
source measured or merely relayed a claim is checkable against its text,
while a model's self-rated confidence is not.

The rule attaches to the column, so it applies to the server's templates too,
since they carry no guidance of their own. The hosted path is unaffected: its
...
notation alone, since a column is for comparing values, and prose is not
comparable. The source's wording is not lost: it is on hover in the UI and in
`_as_written` in exports, next to `_standard_notation`.
...
The converted form is also the dedup identity, so a row written in prose
merges with one that used notation instead of sitting beside it as a separate
fact.
...
non-coding region, since `C to T` is Cys to Thr as readily as cytosine to
thymine.
...
retrieved passage and highlights it. Each citation also carries pilcrow links,
one per supporting passage, so a paper cited through two different chunks
shows both. Source cards display their chunk id and character span.
...
Those ids go straight to the API's `/v1/chunks?ids=` to fetch the passage
text back, along with its `prev_chunk_id` and `next_chunk_id` neighbours.

### How retrieval works

Retrieval defaults to `fused`: the query runs under both `hybrid` and `bm25`
and the two rankings are merged with reciprocal rank fusion.
`--retrieval-mode` takes `fused`, `hybrid`, `vector`, or `bm25`.
...
Gene names, mutation codes, and accessions are exactly the literal tokens
BM25 matches and dense similarity does not, so a curation tool should not
rely on either alone. Fusion costs one extra retrieval call, around 0.3s, and
leaves the top of a good ranking unchanged.
...
but the retrieved context and the answer compete for one context window, so
both are budgeted: the answer is sized first, scaled to the number of
sources and capped at 20k tokens, and the prompt gets what is left.

Neither kind of loss is allowed to pass silently.

- If the answer still hits its limit, the run is flagged truncated, since a
  cut-off table is missing rows and must not read as a complete result.
- If the retrieval does not fit the model's window, the tail is dropped and
  the run reports how many of the requested sources were actually shown.
  Only those are kept as sources, so a citation marker can never point at a
  passage the model never saw.

Window sizes differ: Qwen holds 131k tokens and takes all 100 sources; Llama
holds 60k and shows about 79 of them, which it now says. At `--top-k 100`
expect roughly 20s rather than 2s.
...
Every row carries `_`-prefixed columns recording what produced it: template
id, version and hash, model, collection, `top_k`, retrieval mode, UTC
timestamp, the server's request id, and on the local path, the generator, its
endpoint, and the prompt hash. A table can be traced back to the exact
configuration that generated it. Use `--no-provenance` for a clean table.

## Notes on this deployment

- The knowledge-graph backend reports `{"backend": "disabled"}`, so LitRAG
  never sets `use_graph`. The original demo's toggle had no effect.
...
- Two collections are available; see [Choosing collections](#choosing-collections) above.
- Only read-only endpoints are used. Ingest, collection management, grading,
  and admin endpoints exist on the API but are deliberately out of scope.
...
To add or change a dependency, edit `litrag/pyproject.toml` (or use
`uv add <pkg>` or `uv add --dev <pkg>`) and commit the updated `uv.lock`.

With pip:

```bash
pip install -e "litrag[dev]"
pytest
```

Tests run against recorded API responses in `litrag/tests/fixtures/`. Every
data-quality case is pinned to something a live query actually returned.

## Where to read next

- [ARCHITECTURE.md](ARCHITECTURE.md): how the package is wired, for maintainers.
- [WALKTHROUGH.md](WALKTHROUGH.md): what a curated table is and how to audit a claim, no install needed.
- [litrag/README.md](litrag/README.md): the package's own short README.
